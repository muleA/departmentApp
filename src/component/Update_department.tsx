...
export default function UpdateDepartmentForm(props:{visibility:boolean;
visibilityToggler:(visible:boolean)=>void; selectedDepartment:Department})
{
    ...
    const [isUpdateModaleVisible, setIsUpdateModalVisible] = useState(props.visibility)
    /* event handler */
    const showModal = () => {
    ...
    const onFinish: (value: {
        name: string;description: string; parentDepartmentId?: string; }) => void = (value) => {
       setSubmitLoading(true)
            const asyncDispatcher = async ()=>await dispatch(updateDepartment({id:props.selectedDepartment.id,...value} as Department));
