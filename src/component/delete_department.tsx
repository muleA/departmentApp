    ...
    /*   const handleOk = () => {
      ...
      const ConfirmDelete: (department:Department)=>void = (department) =>{
        ...
        const asyncDispatcher = async ()=>{
          await dispatch(deleteDepartment(
            department,getDescendents(department.id)));
      ...
      return(
    <Modal  className="text-center mx-auto "
    title="Delete Modal"
    onCancel={handleCancel}
    closeIcon= "X"
    visible={props.modalVisibility}
    footer={[  
    <Button className="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:ring-gray-300 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600" key="back" onClick={handleCancel}>
    Cancel
  </Button>,
  <Button className="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2" key="submit" type="primary" loading={deleteLoading} onClick={()=>ConfirmDelete(props.selectedDepartment!)}>
    Ok
  </Button>]}
  >
